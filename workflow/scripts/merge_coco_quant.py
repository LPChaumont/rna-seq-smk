...
for i, file in enumerate(sorted(snakemake.input.quants)):
    ...
    if i == 0:
        counts_df = df[["gene_id", "gene_name", "gene_biotype", "count"]].rename(
            columns={"count": sample}
        )
        cpm_df = df[["gene_id", "gene_name", "gene_biotype", "cpm"]].rename(
            columns={"cpm": sample}
        )
        tpm_df = df[["gene_id", "gene_name", "gene_biotype", "tpm"]].rename(
            columns={"tpm": sample}
        )
