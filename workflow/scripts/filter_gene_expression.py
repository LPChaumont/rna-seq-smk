...
def filter_gene_expression(
    file_path, condition_dict, min_gene_expr=1, min_condition_expr=1, save=False
):
    df = pd.read_table(file_path)
    non_numeric_cols = df.select_dtypes(exclude="number").columns.tolist()

    melted_df = (
        df.melt(id_vars=non_numeric_cols, var_name="sample", value_name="abundance")
    ...
    mean_df = (
        melted_df.pivot(index=non_numeric_cols, columns="condition", values="abundance")
        .reset_index()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Filter gene expression data")
    parser.add_argument(
        "-i",
        "--input",
        type=str,
