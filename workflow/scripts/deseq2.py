...
def parse_args():
    parser = argparse.ArgumentParser(description="Arguments for the analysis script.", add_help=False)
...
def design_in_sample_sheet(design, samples):
    ...
    if missing_var:
        print(f"Error: The following variables are missing in the sample sheet columns: {', '.join(missing_var)}", file=sys.stderr)
        sys.exit(1)
...
def validate_sample_ids(samples, counts):
    ...
    if len(sample_id_diff) != 0:
        print("Error: Sample IDs in the sample sheet and the counts file are not the same", file=sys.stderr)
        sys.exit(1)

def validate_contrasts(samples, contrasts):
    samples_df = pd.read_table(samples)
    countrasts_df = pd.read_table(contrasts)
    contrast_groups = pd.unique(countrasts_df[['reference_level', 'tested_level']].values.ravel())
    samples_df_groups = pd.unique(samples_df['condition'])
    
    if set(contrast_groups).difference(samples_df_groups):
        print("Conditions in the sample sheet and contrast sheet are not the same", file=sys.stderr)
        sys.exit(1)
